...
regions <- unique(m_master$region_s)
...
m_master <- m_master %>% mutate(month = month - 1) %>% mutate(year = ifelse(month == 0, year - 1, year)) %>% mutate(month = ifelse(month == 0,12, month))
...
data <- data %>% 
  group_by(fips, measure = num_90, monthyear, county = county.x, income group, poverty_group, population_est, state, year, region_s) %>% 
...
data <- data %>% 
  group_by(fips, measure = num_90, monthyear, county = county.x, income group, poverty_group, population_est, state, year) %>% 
  summarise(deaths = sum(deaths, na.rm = T)) %>%
  dplyr::filter(is.finite(measure)) %>%
  mutate(deaths = (deaths/as.numeric(population_est))*100000)
data <- na.omit(data)
