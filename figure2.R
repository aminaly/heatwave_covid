...
plot_data_bin5 <- function(data, plot_title, xlab="Temp (C)", ylab = "# Visitors / Home Devices", summer = F) {
  ...
  #separate the three datasets
  data_1 <- data_s %>% filter(income_group == 1)
  data_2 <- data_s %>% filter(income_group == 2)
  data_3 <- data_s %>% filter(income_group == 3)
  data_4 <- data_s %>% filter(income_group == 4)
  data_5 <- data_s %>% filter(income_group == 5)
  ...
  model_3 <- fe_model(data_3, level = LVL)
  model_4 <- fe_model(data_4, level = LVL)
  model_5 <- fe_model(data_5, level = LVL)
  ...
  #bootstrap the data by doing the model however many times, getting the coefficient values (3)
  boots_5 <- bootstrap_data(data_5, short=T, level= LVL)
  boots_5 <- boots_5[-1,]
  boots_5 <- as_tibble(boots_5) %>% mutate(xvar_bin1 = 0) %>% relocate(xvar_bin1)
  conf_5 <- apply(boots_4,2,function(x) quantile(x,probs=c(0.05,0.5,0.95), na.rm = T)) 
  conf_5 <- as.data.frame(cbind(1:BINS, conf_4[1,], conf_4[2,], conf_4[3,]))
  colnames(conf_5) <- c("term", "low", "mid", "upper")   
  conf_5$term <- row.names(conf_5)
  
  coefs_5 <- tidy(model_5, conf.int = T)
  coefs_5 <- coefs_5[grepl("xvar_bin", coefs_5$term),]
  coefs1_5 <- coefs_5[1,] %>% mutate(term = "xvar_bin1", estimate = 0, std.error = 0, statistic = 0, p.value = 0, conf.low = 0, conf.high = 0)
  coefs_5 <- rbind(coefs1_5, coefs_5)
  coefs_5$grp <- "5"
  coefs_5 <- left_join(coefs_5, conf_5, by = "term")
  
  coefs <- rbind(coefs_1, coefs_2, coefs_3, coefs_4, coefs_5)
...
plot_title <- paste0("Mobility Index v Avg High Year ", ig)
plot_data_bin4(data, plot_title, xlab = "high_temp (0-40C)")

## plot binned data summer only 
plot_title <- paste0("Mobility Index v Avg High Summer Year ", ig)
